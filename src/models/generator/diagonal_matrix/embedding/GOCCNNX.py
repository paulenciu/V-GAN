...
class GOCCNNX(AbstractGenerator):

    def __init__(self, latent_size, image_shape, model_z_dim=100):
        super().__init__()
        self.latent_size = latent_size
        self.model_z_dim = model_z_dim
    ...
    def _build(self, conv_dim=32):

        self.linear_input = nn.Linear(self.latent_size, self.model_z_dim)

        self.conv_blocks = nn.Sequential(
            # Input layer (model_z_dim -> conv_dim*8)
            nn.ConvTranspose2d(self.model_z_dim, conv_dim * 8, kernel_size=4, stride=1, padding=0, bias=False),
    ...
    def forward(self, input, mode="train"):
        ...
        x = x.view(x.size(0), self.model_z_dim, 1, 1)
