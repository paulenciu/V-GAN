...
class GeneratorThreeChannelV2(AbstractGenerator):

    def __init__(self, latent_size, image_shape):
        super(GeneratorThreeChannelV2, self).__init__()

        self._noise_dim = torch.tensor([latent_size])
        self._img_shape = image_shape

        img_size = image_shape[2] * image_shape[2] * image_shape[0]
    ...
    def sample_subspace_masks(self, noise, mode="train"):
        return self.forward(noise, mode).view(-1, self._img_shape[0], self._img_shape[1], self._img_shape[2])