...
class VMMDSingleMask(VMMD):
    ...
    def fit(self, X, autoencoder):
        ...
        ndims = int(X.shape[1] / 3)
        train_size = X.shape[0]
        self.batch_size = min(self.batch_size, train_size)

        device = self.device

        generator = GeneratorSingleMaskRes(latent_size=latent_size, img_size=ndims).to(device)
        ...
        for epoch in range(epochs):
            ...
            for batch in tqdm(data_loader, leave=False):
                ...
                fake_subspaces = fake_subspaces.repeat(1, 3) #stacks the masks upon each other three times
