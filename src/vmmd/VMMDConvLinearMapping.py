...
class VMMDConvLinearMapping(VMMD):
    ...
    def check_if_myopic(self, x_data, emb_func, bandwidth: Union[float, np.array] = 0.01, count=500) -> pd.DataFrame:
        ...
        u_subspaces = self.generate_subspaces(count).repeat(1,3,1,1)
        ...
        if not hasattr(self, 'bandwidth'):
            mmd_loss = MMDLossConstrained(0, unflattened=True)
            mmd_loss.forward(
    ...
    def fit(self, X, autoencoder):
        ...
        loss_function = MMDLossConstrained(weight=10, kernel=RBF(), unflattened=True)
        ...
        for epoch in range(epochs):
            ...
            # GET NOISE TENSORS#
            generator_input_shape = generator.noise_dim
            if cuda:  # Need to open this if statement as the Tensor function has to be called from diferent modules depending of the device
                noise_tensor = torch.cuda.FloatTensor(
                    self.batch_size, generator_input_shape[0], generator_input_shape[1], generator_input_shape[2]).to(torch.device('cuda'))
            elif mps:
                noise_tensor = torch.mps.Tensor(
                    self.batch_size, generator_input_shape[0], generator_input_shape[1], generator_input_shape[2]).to(torch.device('mps'))
            else:
                noise_tensor = torch.Tensor(self.batch_size, generator_input_shape[0], generator_input_shape[1], generator_input_shape[2])
    ...
    def generate_subspaces(self, nsubs):
        ...
        noise_tensor = torch.Tensor(nsubs, generator_input_shape[0], generator_input_shape[1], generator_input_shape[2]).to('cpu')
