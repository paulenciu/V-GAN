...
class VMMDLinearMapping(VMMD):
    ...
    def check_if_myopic(self, x_data, emb_func, bandwidth: Union[float, np.array] = 0.01, count=500) -> pd.DataFrame:
        ...
        u_subspaces = self.generate_subspaces(count).to(torch.float32).to(device=self.device)

        ux_sample = self.apply_linear_transformation(u_subspaces, x_sample)
        ...
        axs[0].imshow(tensor_to_image(x_sample[0]))
        axs[1].imshow(tensor_to_image(ux_sample[0]))
    ...
    def fit(self, X, autoencoder):
        ...
        for epoch in range(epochs):
            ...
            for batch in tqdm(data_loader, leave=False):
                ...
                processed_fake_subspaces = self.apply_linear_transformation(fake_subspaces, batch)
    ...
    def apply_linear_transformation(self, fake_subspaces, batch):
        ...
        return torch.bmm(fake_subspaces, batch)  # Shape: (32, 3, 32, 32)
