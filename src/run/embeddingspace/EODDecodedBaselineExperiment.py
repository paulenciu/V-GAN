...
class EODDecodedBaselineExperiment:

    def __init__(self, dataset_type, category, image_size_od, standardize_data, preprocessing_fn, od_models, encoder,
                 encoder_name, root_dir="../experiments/od_baselines/embeddingspace/decoded"):
    ...
    def calculate_od_stats(self, od_model, y_test, decision_scores):
        return {"Dataset": self.dataset_type,
                ...
                "OD Method": str(od_model.base_estimator.__class__.__name__) if isinstance(od_model,
                                                                                           FeatureBagging) else str(
                    od_model.__class__.__name__), }